...
int virg_init(virginian *v)
{
	...
	for(i = 0; i < VIRG_MEM_TABLETS; i++) {
		...
		r = cudaHostAlloc((void**)&v->tablet_slots[i], sizeof(virg_tablet_meta), cudaHostAllocMapped);
		VIRG_CUDCHK("Allocating pinned tablet memory");
#else
		v->tablet_slots[i] = malloc(sizeof(virg_tablet_meta));
		...
		memset(v->tablet_slots[i], 0xDEADBEEF, sizeof(virg_tablet_meta));
#endif
	}
