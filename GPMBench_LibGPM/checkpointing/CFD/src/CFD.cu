...
int main(int argc, char** argv)
{
    ...
    gpmcp *cp_euler;
    if(!RECOVER)
        cp_euler = gpmcp_create("./cp_euler", nelr * NVAR * sizeof(float), 1, 1);
    else
        cp_euler = gpmcp_open("./cp_euler");
    ...
    gpmcp_register(cp_euler, variables, nelr * NVAR * sizeof(float), 0);
    ...
    long long recover_time = 0, timer2 = 0, timer3 = 0;
    sdkCreateTimer(&timer);
    sdkStartTimer(&timer); 
    if(RECOVER) {
        cudaDeviceSynchronize();
        auto start_recovery = std::chrono::high_resolution_clock::now();
        gpmcp_restore(cp_euler, 0);
        cudaDeviceSynchronize();
    ...
    ddio_off(); 
        ...
        for(long long i = 0; i < iterations; i++)
        {
            ...
            if (i % CP_ITER == CP_ITER - 1) {
                ...
                gpmcp_checkpoint(cp_euler, 0);
#if defined(NVM_ALLOC_GPU) && !defined(EMULATE_NVM)
                //download(h_variables, variables, nelr*NVAR);
                //cudaMemcpy((void **) old_variables, (void **)variables, nelr * NVAR, cudaMemcpyDeviceToHost);
#endif
                cudaDeviceSynchronize();
	...
	std::cout << "CheckpointTime\t" << (double)timer2 / 1000000.0 << "\n";
	std::cout << "PersistTime\t" << persist_time / 1000000.0 << "\n";
