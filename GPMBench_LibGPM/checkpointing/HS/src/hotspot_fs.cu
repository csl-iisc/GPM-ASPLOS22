...
void checkpoint(int fd, float *vect, int grid_rows, int grid_cols) 
{
    float *h_vect = new float[grid_rows * grid_cols];
    auto start = TIME_NOW; 
    cudaMemcpy(h_vect, vect, sizeof(float) * grid_rows * grid_cols, cudaMemcpyDeviceToHost);
    memcpy_time += (TIME_NOW - start).count();
    start = TIME_NOW; 
    if(pmem_write(fd, h_vect, sizeof(float) * grid_rows * grid_cols, 0) 
        != sizeof(float) * grid_rows * grid_cols)
...
void restore(int fd, float *vect, int grid_rows, int grid_cols)
{
    float *h_vect = new float[grid_rows * grid_cols];
    if(pread(fd, vect, sizeof(float) * grid_rows * grid_cols, 0) 
        != sizeof(float) * grid_rows * grid_cols)
        printf("Error restoring data\n");
    cudaMemcpy(vect, h_vect, sizeof(float) * grid_rows * grid_cols, cudaMemcpyHostToDevice);
	delete h_vect;
}
...
int compute_tran_temp(float *MatrixPower,float *MatrixTemp[2], int col, int row, \
		int total_iterations, int num_iterations, int blockCols, int blockRows, int borderCols, int borderRows, int fd) 
{
	...
	for (int t = 0; t < total_iterations; t+=num_iterations) {
		...
		col,row,borderCols, borderRows, Cap,Rx,Ry,Rz,step,total_iterations);
		cudaDeviceSynchronize();
		
		if((t / num_iterations) % CP_ITER == CP_ITER - 1) {
		    checkpoint(fd, MatrixTemp[dst], col, row);
		}
	}
    return dst;
}
...
int main(int argc, char** argv)
{
    ...
    int fd = open("/pmem/cp_hotspot_fs.out", O_CREAT | O_RDWR, S_IRWXU | S_IRWXG | S_IRWXO);
    auto timer_start = std::chrono::high_resolution_clock::now();
    int ret;
    if(RESTORE) {
        ret = 0;
        restore(fd, MatrixTemp[0], grid_cols, grid_rows);
    }        
    else {
        ret = compute_tran_temp(MatrixPower,MatrixTemp,grid_cols,grid_rows, \
	        total_iterations,pyramid_height, blockCols, blockRows, borderCols, borderRows, fd);
	}
	cudaDeviceSynchronize();
