...
#define DEFINE_string(flag, default_value, description) const std::string FLAGS_##flag ((default_value))
 ...
 int *dpoints = (int*) malloc(sizeof(int));
 int *d_dpoints;
...
int main(int argc, char **argv)
{
    ...
    cudaMalloc((void**)&d_dpoints, sizeof(int));
    cudaMemset(d_dpoints, 0, sizeof(int));
...
//START_BW_MONITOR
    ...
    for (int iter = 0; iter < FLAGS_iterations; ++iter)
    {
        ...
        if (iter%CP_ITER == CP_ITER-1) {
            ...
            conv1_ssf << conv1_file << "-temp.bin";
            conv1_ssbf << conv1_file << "-temp.bias.bin";
            ...
            conv2_ssf << conv2_file << "-temp.bin";
            conv2_ssbf << conv2_file << "-temp.bias.bin";
            ...
            fc1_ssf << fc1_file << "-temp.bin";
            fc1_ssbf << fc1_file << "-temp.bias.bin";
            ...
            fc2_ssf << fc2_file << "-temp.bin";
            fc2_ssbf << fc2_file << "-temp.bias.bin";
            ...
            if(is_pmem) {
                pmem_persist(conv1_fp1, sizeof(float) * conv1.pconv.size());
                pmem_persist(conv1_fp2, sizeof(float) * conv1.pbias.size());
                pmem_persist(conv2_fp1, sizeof(float) * conv2.pconv.size());
                pmem_persist(conv2_fp2, sizeof(float) * conv2.pbias.size());
                pmem_persist(fc1_fp1, sizeof(float) * fc1.pneurons.size());
                pmem_persist(fc1_fp2, sizeof(float) * fc1.pbias.size());
                pmem_persist(fc2_fp1, sizeof(float) * fc2.pneurons.size());
                pmem_persist(fc2_fp2, sizeof(float) * fc2.pbias.size());
            }
            else {
            ...
            rename("/pmem/conv1-temp.bin", "/pmem/conv1.bin");
            rename("/pmem/conv1-temp.bias.bin", "/pmem/conv1.bias.bin");
            rename("/pmem/conv2-temp.bin", "/pmem/conv2.bin");
            rename("/pmem/conv2-temp.bias.bin", "/pmem/conv2.bias.bin");
            rename("/pmem/ip1-temp.bin", "/pmem/ip1.bin");
            rename("/pmem/ip1-temp.bias.bin", "/pmem/ip1.bias.bin");
            rename("/pmem/ip2-temp.bin", "/pmem/ip2.bin");
            rename("/pmem/ip2-temp.bias.bin", "/pmem/ip2.bias.bin");
...
//STOP_BW_MONITOR
    ...
    FILE *fptr = fopen("execution_time.csv", "a");

    fprintf(fptr, "%f\t%f\n", std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() / 1000.0f, (double)timings * 1000.0 / CLOCKS_PER_SEC);

    fclose(fptr);

    printf("Iteration time: %f ms\n", std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() / 1000.0f / FLAGS_iterations);
    printf("Checkpoint time: %f ms\n", (double)timings / 1000000.0f);
