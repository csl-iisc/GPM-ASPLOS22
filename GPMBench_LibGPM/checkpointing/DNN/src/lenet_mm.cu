...
double memcpy_time = 0, pwrite_time = 0, persist_time = 0; 
...
struct ConvBiasLayer
{
    ...
    void ToFile(void *wfp, void *bfp)
    {
        ...
        auto start = TIME_NOW; 
        memcpy(wfp, &pconv[0], sizeof(float) * in_channels * out_channels * kernel_size * kernel_size);
        memcpy_time += (TIME_NOW - start).count(); 
        start = TIME_NOW; 
        if(pmem_is_pmem(wfp, sizeof(float) * in_channels * out_channels * kernel_size * kernel_size))
            pmem_persist(wfp, sizeof(float) * in_channels * out_channels * kernel_size * kernel_size);
        else
            pmem_msync(wfp, sizeof(float) * in_channels * out_channels * kernel_size * kernel_size);
        persist_time += (TIME_NOW - start).count(); 
        nvm_writes += sizeof(float) * in_channels * out_channels * kernel_size * kernel_size; 
        ...
        start = TIME_NOW; 
        memcpy(bfp, &pbias[0], sizeof(float) * out_channels);
        memcpy_time += (TIME_NOW - start).count(); 
        start = TIME_NOW; 
        if(pmem_is_pmem(bfp, sizeof(float) * out_channels))
            pmem_persist(bfp, sizeof(float) * out_channels);
        else
            pmem_msync(bfp, sizeof(float) * out_channels);
        persist_time += (TIME_NOW - start).count();
        nvm_writes +=  sizeof(float) * out_channels; 
    }
};
...
struct FullyConnectedLayer
{
    ...
    void ToFile(void *wfp, void *bfp)
    {
        ...
        memcpy(wfp, &pneurons[0], sizeof(float) * inputs * outputs);
        if(pmem_is_pmem(wfp, sizeof(float) * inputs * outputs))
            pmem_persist(wfp, sizeof(float) * inputs * outputs);
        else
            pmem_msync(wfp, sizeof(float) * inputs * outputs);
        ...
        memcpy(bfp, &pbias[0], sizeof(float) * outputs);
        if(pmem_is_pmem(bfp, sizeof(float) * outputs))
            pmem_persist(bfp, sizeof(float) * outputs);
        else
            pmem_msync(bfp, sizeof(float) * outputs);
    }
};
...
struct TrainingContext
{
    ...
    cudnnConvolutionFwdAlgoPerf_t conv1algo, conv2algo;
    cudnnConvolutionBwdFilterAlgoPerf_t conv1bwfalgo, conv2bwfalgo;
    cudnnConvolutionBwdDataAlgoPerf_t conv2bwdalgo;
    ...
    {        
        ...
        checkCUDNN(cudnnConvolutionForward(cudnnHandle, &alpha, dataTensor,
                    ...
                    conv1algo.algo, workspace, m_workspaceSize, &beta,
                    conv1Tensor, conv1));
        ...
        checkCUDNN(cudnnConvolutionForward(cudnnHandle, &alpha, pool1Tensor,
                    ...
                    conv2algo.algo, workspace, m_workspaceSize, &beta,
                    conv2Tensor, conv2));
    ...
    size_t SetBwdConvolutionTensors(cudnnTensorDescriptor_t& srcTensorDesc, cudnnTensorDescriptor_t& dstTensorDesc,
            cudnnFilterDescriptor_t& filterDesc, cudnnConvolutionDescriptor_t& convDesc, 
            cudnnConvolutionBwdFilterAlgoPerf_t *falgo, cudnnConvolutionBwdDataAlgoPerf_t *dalgo)
    {
    ...
    {    
        ...
        checkCUDNN(cudnnConvolutionBackwardFilter(cudnnHandle, &alpha, pool1Tensor,
                    ...
                    conv2bwfalgo.algo, workspace, m_workspaceSize,
        ...
        checkCUDNN(cudnnConvolutionBackwardData(cudnnHandle, &alpha, conv2filterDesc,
                    ...
                    conv2bwdalgo.algo, workspace, m_workspaceSize,
        ...
        checkCUDNN(cudnnConvolutionBackwardFilter(cudnnHandle, &alpha, dataTensor,
                    ...
                    conv1bwfalgo.algo, workspace, m_workspaceSize,
...
int main(int argc, char **argv)
{
    ...
    if (FLAGS_pretrained)
    {
        ...
        bRet = conv1.FromFile("/pmem/conv1");
        bRet &= conv2.FromFile("/pmem/conv2");
        bRet &= fc1.FromFile("/pmem/ip1");
        bRet &= fc2.FromFile("/pmem/ip2");
        printf("Time to restore weights: %f ms\n", std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count() / 1000.0f);
    }
...
//START_BW_MONITOR
    ...
    const char *conv1_file = "/pmem/conv1";
    std::stringstream conv1_ssf, conv1_ssbf;
    conv1_ssf << conv1_file << ".bin";
    conv1_ssbf << conv1_file << ".bias.bin";
    void *conv1_fp1 = pmem_map_file(conv1_ssf.str().c_str(), conv1.pconv.size() * sizeof(float), PMEM_FILE_CREATE, S_IRWXU | S_IRWXG | S_IRWXO, &file_size, &is_pmem );
    void *conv1_fp2 = pmem_map_file(conv1_ssbf.str().c_str(), conv1.pbias.size() * sizeof(float), PMEM_FILE_CREATE, S_IRWXU | S_IRWXG | S_IRWXO, &file_size, &is_pmem );
    
    const char *conv2_file = "/pmem/conv2";
    std::stringstream conv2_ssf, conv2_ssbf;
    conv2_ssf << conv2_file << ".bin";
    conv2_ssbf << conv2_file << ".bias.bin";
    void *conv2_fp1 = pmem_map_file(conv2_ssf.str().c_str(), conv2.pconv.size() * sizeof(float), PMEM_FILE_CREATE, S_IRWXU | S_IRWXG | S_IRWXO, &file_size, &is_pmem );
    void *conv2_fp2 = pmem_map_file(conv2_ssbf.str().c_str(), conv2.pbias.size() * sizeof(float), PMEM_FILE_CREATE, S_IRWXU | S_IRWXG | S_IRWXO, &file_size, &is_pmem );
    
    const char *fc1_file = "/pmem/ip1";
    std::stringstream fc1_ssf, fc1_ssbf;
    fc1_ssf << fc1_file << ".bin";
    fc1_ssbf << fc1_file << ".bias.bin";
    void *fc1_fp1 = pmem_map_file(fc1_ssf.str().c_str(), fc1.pneurons.size() * sizeof(float), PMEM_FILE_CREATE, S_IRWXU | S_IRWXG | S_IRWXO, &file_size, &is_pmem );
    void *fc1_fp2 = pmem_map_file(fc1_ssbf.str().c_str(), fc1.pbias.size() * sizeof(float), PMEM_FILE_CREATE, S_IRWXU | S_IRWXG | S_IRWXO, &file_size, &is_pmem );
    
    const char *fc2_file = "/pmem/ip2";
    std::stringstream fc2_ssf, fc2_ssbf;
    fc2_ssf << fc2_file << ".bin";
    fc2_ssbf << fc2_file << ".bias.bin";
    void *fc2_fp1 = pmem_map_file(fc2_ssf.str().c_str(), fc2.pneurons.size() * sizeof(float), PMEM_FILE_CREATE, S_IRWXU | S_IRWXG | S_IRWXO, &file_size, &is_pmem );
    void *fc2_fp2 = pmem_map_file(fc2_ssbf.str().c_str(), fc2.pbias.size() * sizeof(float), PMEM_FILE_CREATE, S_IRWXU | S_IRWXG | S_IRWXO, &file_size, &is_pmem );
    ...
    for (int iter = 0; iter < FLAGS_iterations; ++iter)
    {
        ...
        if (iter%CP_ITER == CP_ITER-1) {
            ...
            //START_BW_MONITOR2("bw_mm_cudnn.csv"); 
            auto start1 = TIME_NOW; 
            checkCudaErrors(cudaMemcpy(conv1_fp1, d_pconv1, sizeof(float) * conv1.pconv.size(), cudaMemcpyDeviceToHost));
            checkCudaErrors(cudaMemcpy(conv1_fp2, d_pconv1bias, sizeof(float) * conv1.pbias.size(), cudaMemcpyDeviceToHost));
            checkCudaErrors(cudaMemcpy(conv2_fp1, d_pconv2, sizeof(float) * conv2.pconv.size(), cudaMemcpyDeviceToHost));
            checkCudaErrors(cudaMemcpy(conv2_fp2, d_pconv2bias, sizeof(float) * conv2.pbias.size(), cudaMemcpyDeviceToHost));
            checkCudaErrors(cudaMemcpy(fc1_fp1, d_pfc1, sizeof(float) * fc1.pneurons.size(), cudaMemcpyDeviceToHost));
            checkCudaErrors(cudaMemcpy(fc1_fp2, d_pfc1bias, sizeof(float) * fc1.pbias.size(), cudaMemcpyDeviceToHost));
            checkCudaErrors(cudaMemcpy(fc2_fp1, d_pfc2, sizeof(float) * fc2.pneurons.size(), cudaMemcpyDeviceToHost));
            checkCudaErrors(cudaMemcpy(fc2_fp2, d_pfc2bias, sizeof(float) * fc2.pbias.size(), cudaMemcpyDeviceToHost));
            memcpy_time += (TIME_NOW - start1).count(); 
            //STOP_BW_MONITOR
            start1 = TIME_NOW; 
            if(is_pmem) {
                pmem_mt_persist(conv1_fp1, sizeof(float) * conv1.pconv.size());
                pmem_mt_persist(conv1_fp2, sizeof(float) * conv1.pbias.size());
                pmem_mt_persist(conv2_fp1, sizeof(float) * conv2.pconv.size());
                pmem_mt_persist(conv2_fp2, sizeof(float) * conv2.pbias.size());
                pmem_mt_persist(fc1_fp1, sizeof(float) * fc1.pneurons.size());
                pmem_mt_persist(fc1_fp2, sizeof(float) * fc1.pbias.size());
                pmem_mt_persist(fc2_fp1, sizeof(float) * fc2.pneurons.size());
                pmem_mt_persist(fc2_fp2, sizeof(float) * fc2.pbias.size());
                nvm_writes +=  sizeof(float) * conv1.pconv.size() + sizeof(float) * conv1.pbias.size() + sizeof(float) * conv2.pconv.size() +  sizeof(float) * conv2.pbias.size() +  sizeof(float) * fc1.pneurons.size() + sizeof(float) * fc1.pbias.size() +  sizeof(float) * fc2.pneurons.size() +  sizeof(float) * fc2.pbias.size(); 
            }
            else {
                pmem_msync(conv1_fp1, sizeof(float) * conv1.pconv.size());
                pmem_msync(conv1_fp2, sizeof(float) * conv1.pbias.size());
                pmem_msync(conv2_fp1, sizeof(float) * conv2.pconv.size());
                pmem_msync(conv2_fp2, sizeof(float) * conv2.pbias.size());
                pmem_msync(fc1_fp1, sizeof(float) * fc1.pneurons.size());
                pmem_msync(fc1_fp2, sizeof(float) * fc1.pbias.size());
                pmem_msync(fc2_fp1, sizeof(float) * fc2.pneurons.size());
                pmem_msync(fc2_fp2, sizeof(float) * fc2.pbias.size());
            }
            persist_time += (TIME_NOW - start1).count(); 
            // Now save data
            //printf("Saving data to file\n");
            /*conv1.ToFile(conv1_fp1, conv1_fp2);
            conv2.ToFile(conv2_fp1, conv2_fp2);
            fc1.ToFile(fc1_fp1, fc1_fp2);
            fc2.ToFile(fc2_fp1, fc2_fp2);*/
