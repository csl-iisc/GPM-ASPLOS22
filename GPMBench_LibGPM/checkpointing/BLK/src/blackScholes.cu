...
int main(int argc, char **argv)
{
    ...
    *d_OptionYears;
    ...
    gpmcp *cp_blackScholes;
    if(RESTORE)
    {
        cp_blackScholes = gpmcp_open("./cp_blackscholes.out");
    }
    else
        cp_blackScholes = gpmcp_create("./cp_blackscholes.out", 2 * OPT_SZ, 2, 1 );
    
    gpmcp_register(cp_blackScholes, d_CallResult, OPT_SZ, 0);
    gpmcp_register(cp_blackScholes, d_PutResult, OPT_SZ, 0);

    printf("Executing Black-Scholes GPU kernel (%lli iterations)...\n", NUM_ITERATIONS);
    ...
    if(RESTORE)
    {
        ...
        gpmcp_restore(cp_blackScholes, 0);
        cudaDeviceSynchronize();
