...
int main(int argc, char **argv)
{
    ...
    *d_OptionYears;
    ...
    int is_pmem;
    size_t file_size;
    void *chkpt = pmem_map_file( "/pmem/cp_blackscholes_mm.out", 2 * OPT_SZ, PMEM_FILE_CREATE, S_IRWXU | S_IRWXG | S_IRWXO, &file_size, &is_pmem );
    ...
    double persist_time = 0, memcpy_time = 0; 

    if(RESTORE) {
        checkCudaErrors(cudaMemcpy(d_CallResult, (char*)chkpt, OPT_SZ, cudaMemcpyHostToDevice));
        checkCudaErrors(cudaMemcpy(d_PutResult,  (char*)chkpt + OPT_SZ,  OPT_SZ, cudaMemcpyHostToDevice));
    }
    else {
        for (i = 0; i < NUM_ITERATIONS; i++)
        {
            ...
            if (i % CP_ITER == CP_ITER - 1) {
                //START_BW_MONITOR2("bw_mm_blackScholes.csv");
                auto start = TIME_NOW; 
                checkCudaErrors(cudaMemcpy(chkpt, d_CallResult, OPT_SZ, cudaMemcpyDeviceToHost));
                checkCudaErrors(cudaMemcpy((char*)chkpt + OPT_SZ,  d_PutResult,  OPT_SZ, cudaMemcpyDeviceToHost));
                memcpy_time += (TIME_NOW - start).count(); 
                start = TIME_NOW; 
                if(is_pmem)
                    pmem_mt_persist(chkpt, OPT_SZ * 2);
                else
                    pmem_msync(chkpt, OPT_SZ * 2);
    ...
    printf("execution time: %f \t\n", gpuTime * NUM_ITERATIONS - memcpy_time/1000000.0f - persist_time/1000000.0f);
    ...
    printf("BlackScholes, Throughput = %.4f GOptions/s, Time = %.5f s, Size = %lld options, NumDevsUsed = %u, Workgroup = %u\n",
    ...
    cudaMemcpy(dpoints, d_dpoints, sizeof(int), cudaMemcpyDeviceToHost);
