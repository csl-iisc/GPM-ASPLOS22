...
void checkpoint(float* variables, int nelr)
{
    int is_pmem;
    size_t size;
    void *chkpt = pmem_map_file("/pmem/cp_euler_fs-temp", nelr*NVAR * sizeof(float), PMEM_FILE_CREATE, S_IRWXU | S_IRWXG | S_IRWXO, &size, &is_pmem);
    download((float*)chkpt, variables, nelr*NVAR);
    if(is_pmem)
        pmem_persist(chkpt, nelr*NVAR * sizeof(float));
    else
        pmem_msync(chkpt, nelr*NVAR * sizeof(float));
    rename("/pmem/cp_euler_fs-temp", "/pmem/cp_euler_fs");
}
