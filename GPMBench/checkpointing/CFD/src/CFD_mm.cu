...
double memcpy_time = 0, persist_time = 0; 
...
{
    T* t;
    gpuErrchk(cudaMalloc((void**)&t, sizeof(T)*N));
    return t;
}
...
void checkpoint(void *chkpt, float* variables, int nelr, int is_pmem)
{
    //START_BW_MONITOR2("bw_mm_cfd.csv"); 
    auto start = TIME_NOW; 
    download((float*)chkpt, variables, nelr*NVAR);
    memcpy_time += (TIME_NOW - start).count(); 
    start = TIME_NOW; 
    if(is_pmem)
        pmem_mt_persist(chkpt, nelr*NVAR * sizeof(float));
    else
        pmem_msync(chkpt, nelr*NVAR * sizeof(float));
    nvm_writes += nelr * NVAR * sizeof(float); 
...
int main(int argc, char** argv)
{
    ...
    // set far field conditions and load them into constant memory on the gpu
    {
        ...
        checkCudaErrors( cudaMemcpyToSymbol(ff_variable, h_ff_variable, NVAR*sizeof(float), 0, cudaMemcpyHostToDevice));
        //cudaMemcpy(ff_variable, h_ff_variable, NVAR*sizeof(float), cudaMemcpyHostToDevice);
        checkCudaErrors( cudaMemcpyToSymbol(ff_flux_contribution_momentum_x, &h_ff_flux_contribution_momentum_x, sizeof(float3)) );
        //cudaMemcpy(ff_flux_contribution_momentum_x, &h_ff_flux_contribution_momentum_x, sizeof(float3), cudaMemcpyHostToDevice);
        checkCudaErrors( cudaMemcpyToSymbol(ff_flux_contribution_momentum_y, &h_ff_flux_contribution_momentum_y, sizeof(float3)) );
        //cudaMemcpy(ff_flux_contribution_momentum_y, &h_ff_flux_contribution_momentum_y, sizeof(float3),  cudaMemcpyHostToDevice);
        //cudaMemcpy(ff_flux_contribution_momentum_z, &h_ff_flux_contribution_momentum_z, sizeof(float3), cudaMemcpyHostToDevice);
        ...
        //cudaMemcpy(ff_flux_contribution_density_energy, &h_ff_flux_contribution_density_energy, sizeof(float3),  cudaMemcpyHostToDevice);		
    }
    int nel;
    int nelr;
    ...
    //Creating as many partitions as there are number of blocks 
    //Each block now can 
    
    int is_pmem;
    size_t size;
    void *chkpt = pmem_map_file("/pmem/cp_euler_fs", nelr*NVAR * sizeof(float), PMEM_FILE_CREATE, S_IRWXU | S_IRWXG | S_IRWXO, &size, &is_pmem);
    ...
    for(int i = 0; i < iterations; i++)
    {
        ...
        if (i % CP_ITER == CP_ITER - 1) {
            ...
            checkpoint(chkpt, variables, nelr, is_pmem);
    ...
    std::cout  << (sdkGetAverageTimerValue(&timer)) << " ms with cp time of " << (double)timer2 / 1000000.0 << " ms and other time of " << (double)timer3 / 1000000.0 << std::endl;
    std::cout << "CheckpointTime:\t" << memcpy_time/1000000.0f + persist_time/1000000.0f << std::endl; 
    std::cout << "MemcpyTime\t" << memcpy_time/1000000.0f << "\nPersistTime:\t" << persist_time/1000000.0f << std::endl; 
    std::cout << "Tot writes: " << nvm_writes << std::endl; 
