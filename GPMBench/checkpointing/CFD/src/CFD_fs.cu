...
void checkpoint(int fd, float* variables, int nelr)
{
    float* h_variables = new float[nelr*NVAR];
    auto start = TIME_NOW; 
    download(h_variables, variables, nelr*NVAR);
    ...
    start = TIME_NOW; 
    if (pmem_write(fd, h_variables, sizeof(float) * nelr * NVAR, 0) != sizeof(float) * nelr * NVAR)
...
int main(int argc, char** argv)
{
    ...
    int fd = open("/pmem/cp_euler_fs", O_CREAT | O_RDWR, S_IRWXU | S_IRWXG | S_IRWXO);
    ...
    std::cout << "CheckpointTime:\t" << memcpy_time/1000000.0f + pwrite_time/1000000.0f + persist_time/1000000.0f << std::endl;  
    std::cout << "MemcpyTime:\t" << memcpy_time/1000000.0f << "\nPwriteTime\t" << pwrite_time/1000000.0f << "\nPersistTime\t" << persist_time/1000000.0f << std::endl;  
