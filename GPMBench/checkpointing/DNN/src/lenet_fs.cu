...
struct ConvBiasLayer
{
    ...
    void ToFile(FILE *wfp, FILE *bfp)
    {
        ...
        fwrite(&pconv[0], sizeof(float), in_channels * out_channels * kernel_size * kernel_size, wfp);
        pwrite_time += (TIME_NOW - start_time).count(); 
        auto start = TIME_NOW;  
        fflush(wfp);
        fsync(fileno(wfp));
        ...
        start = TIME_NOW; 
        fwrite(&pbias[0], sizeof(float), out_channels, bfp);
...
struct FullyConnectedLayer
{
    ...
    void ToFile(FILE *wfp, FILE *bfp)
    {
        ...
        fseek(wfp, 0, SEEK_SET);
        auto start_time = TIME_NOW; 
        fwrite(&pneurons[0], sizeof(float), inputs * outputs, wfp);
        pwrite_time += (TIME_NOW - start_time).count(); 
        auto start = TIME_NOW;  
        fflush(wfp);
        fsync(fileno(wfp));
        ...
        fseek(bfp, 0, SEEK_SET);
        start_time = TIME_NOW; 
        fwrite(&pbias[0], sizeof(float), outputs, bfp);
        pwrite_time += (TIME_NOW - start_time).count(); 
        start = TIME_NOW;  
        fflush(bfp);
        fsync(fileno(bfp));
...
//START_BW_MONITOR
        ...
        FILE *conv1_fp1 = fopen(conv1_ssf.str().c_str(), "wb");
        FILE *conv1_fp2 = fopen(conv1_ssbf.str().c_str(), "wb");
        ...
        FILE *conv2_fp1 = fopen(conv2_ssf.str().c_str(), "wb");
        FILE *conv2_fp2 = fopen(conv2_ssbf.str().c_str(), "wb");
        
        const char *fc1_file = "/pmem/ip1";
        std::stringstream fc1_ssf, fc1_ssbf;
        fc1_ssf << fc1_file << ".bin";
        fc1_ssbf << fc1_file << ".bias.bin";
        FILE *fc1_fp1 = fopen(fc1_ssf.str().c_str(), "wb");
        FILE *fc1_fp2 = fopen(fc1_ssbf.str().c_str(), "wb");
        
        const char *fc2_file = "/pmem/ip2";
        std::stringstream fc2_ssf, fc2_ssbf;
        fc2_ssf << fc2_file << ".bin";
        fc2_ssbf << fc2_file << ".bias.bin";
        FILE *fc2_fp1 = fopen(fc2_ssf.str().c_str(), "wb");
        FILE *fc2_fp2 = fopen(fc2_ssbf.str().c_str(), "wb");
