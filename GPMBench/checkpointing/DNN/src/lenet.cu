...
struct ConvBiasLayer
{
    ...
    bool FromFile(const char *fileprefix)
    {
        ...
        int dummy = fread(&pconv[0], sizeof(float), in_channels * out_channels * kernel_size * kernel_size, fp);
        fclose(fp);
        ...
        dummy = fread(&pbias[0], sizeof(float), out_channels, fp);
        fclose(fp);
        return true;
    }
...
struct FullyConnectedLayer
{
    ...
    bool FromFile(const char *fileprefix)
    {
        ...
        int dummy = fread(&pneurons[0], sizeof(float), inputs * outputs, fp);
        fclose(fp);
        ...
        dummy = fread(&pbias[0], sizeof(float), outputs, fp);
        fclose(fp);
        return true;
    }
...
int main(int argc, char **argv)
{
    ...
    gpmcp *cp_dnn;
    if (!FLAGS_pretrained) {
        size_t tot_size = conv1.pconv.size() * sizeof(float) + conv2.pconv.size() * sizeof(float) + 
                          conv1.pbias.size() * sizeof(float) + conv2.pbias.size() * sizeof(float) + 
                          fc1.pneurons.size() * sizeof(float) + fc2.pneurons.size() * sizeof(float) + 
                          fc1.pbias.size() * sizeof(float) + fc2.pbias.size() * sizeof(float);
        cp_dnn = gpmcp_create("./cp_lenet", tot_size, 8, 1);
    }
    else {
        cp_dnn = gpmcp_open("./cp_lenet");
    }

    gpmcp_register(cp_dnn, d_pconv1, conv1.pconv.size() * sizeof(float), 0);
    gpmcp_register(cp_dnn, d_pconv2, conv2.pconv.size() * sizeof(float), 0);
    gpmcp_register(cp_dnn, d_pconv1bias, conv1.pbias.size() * sizeof(float), 0);
    gpmcp_register(cp_dnn, d_pconv2bias, conv2.pbias.size() * sizeof(float), 0);
    gpmcp_register(cp_dnn, d_pfc1, fc1.pneurons.size() * sizeof(float), 0);
    gpmcp_register(cp_dnn, d_pfc2, fc2.pneurons.size() * sizeof(float), 0);
    gpmcp_register(cp_dnn, d_pfc1bias, fc1.pbias.size() * sizeof(float), 0);
    gpmcp_register(cp_dnn, d_pfc2bias, fc2.pbias.size() * sizeof(float), 0);

    if (FLAGS_pretrained) {
        cudaDeviceSynchronize();
        auto start = std::chrono::high_resolution_clock::now();
        gpmcp_restore(cp_dnn, 0);
        cudaDeviceSynchronize();
...
//STOP_BW_MONITOR
    ...
    printf("FLAGS_iterations: %d", FLAGS_iterations); 
    printf("CheckpointTime\t%f\tms\n", (double)timings / 1000000.0);
    printf("PersistTime:\t%f\tms\n", (double)persist_time / 1000000.0);
