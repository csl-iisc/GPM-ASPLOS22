...
void checkpoint(void *chkpt, float *vect, int grid_rows, int grid_cols, int is_pmem) 
{
    //START_BW_MONITOR2("bw_mm_hotspot.csv");
    auto start = TIME_NOW; 
    cudaMemcpy(chkpt, vect, sizeof(float) * grid_rows * grid_cols, cudaMemcpyDeviceToHost);
    memcpy_time += (TIME_NOW - start).count(); 
    start = TIME_NOW; 
    if(is_pmem)
        pmem_mt_persist(chkpt, sizeof(float) * grid_rows * grid_cols);
    else
        pmem_msync(chkpt, sizeof(float) * grid_rows * grid_cols);
    nvm_writes += sizeof(float) * grid_rows * grid_cols; 
    persist_time += (TIME_NOW - start).count(); 
    //STOP_BW_MONITOR 
}


void restore(void *chkpt, float *vect, int grid_rows, int grid_cols)
{
    cudaMemcpy(vect, chkpt, grid_rows * grid_cols, cudaMemcpyHostToDevice);
}
...
int compute_tran_temp(float *MatrixPower,float *MatrixTemp[2], int col, int row, \
		int total_iterations, int num_iterations, int blockCols, int blockRows, int borderCols, int borderRows, void *chkpt, int is_pmem) 
{
	...
	for (int t = 0; t < total_iterations; t+=num_iterations) {
		...
		if((t / num_iterations) % CP_ITER == CP_ITER - 1) {
		    checkpoint(chkpt, MatrixTemp[dst], col, row, is_pmem );
		    //checkpoint((char*)chkpt + sizeof(float) * col * row, MatrixTemp[src], col, row, is_pmem );
		    //checkpoint((char*)chkpt + 2*sizeof(float) * col * row, MatrixPower, col, row, is_pmem );
		}
	}
    return dst;
}
...
int main(int argc, char** argv)
{
    ...
    int is_pmem;
    size_t file_size;
    const char* hotspot_pmem_file = "/pmem/cp_hotspot_mm"; 
    void *chkpt = pmem_map_file(hotspot_pmem_file, sizeof(float) * size, PMEM_FILE_CREATE, S_IRWXU | S_IRWXG | S_IRWXO, &file_size, &is_pmem );
    auto timer_start = std::chrono::high_resolution_clock::now();
    int ret;
    if(RESTORE) {
        ret = 0;
        restore(chkpt, MatrixTemp[0], grid_cols, grid_rows);
        //restore((char*)chkpt + sizeof(float) * size, MatrixTemp[1], grid_cols, grid_rows);
        //restore((char*)chkpt + 2*sizeof(float) * size, MatrixPower, grid_cols, grid_rows);
    }        
    else {
        ret = compute_tran_temp(MatrixPower,MatrixTemp,grid_cols,grid_rows, \
	        total_iterations,pyramid_height, blockCols, blockRows, borderCols, borderRows, chkpt, is_pmem);
	}
	cudaDeviceSynchronize();
