...
bool shuffle_simple_test(int argc, char **argv)
{
    ...
    int fd_result = open("/pmem/pm_scan_result",  O_CREAT | O_RDWR, S_IRWXU | S_IRWXG | S_IRWXO);
    ...
    for(int iter = 0; iter < n_arrays; ++iter) {
        ...
        start = TIME_NOW;
        long long len = pmem_write(fd_result, h_result, sz, iter * sizeof(int) * n_elements);
        if(len != sz)
            printf("Error, all data not written to file, %lld of %lu written\n", len, sz);
    ...
    printf("\nruntime: %f ms\n", (operation_time + persist_time + memcpy_time + pwrite_time)/1000000.0);
    return 0;
}
