...
/// shortcut for 2^30
#define VIRG_GB			(size_t)1073741824
...
#define VIRG_TABLET_INFO_SIZE		256
...
/// maximum persistent file size
#define VIRG_MAX_FILE_SIZE		(3 * VIRG_GB)

/// tablet slots to allocate in memory
#define VIRG_MEM_TABLETS		256
...
#define VIRG_CUDCHKCALL(call, desc)											   \
	...
	if((r = call) != cudaSuccess) {											   \
		fprintf(stderr, "CUDA %s error (%d): %s :: %s line %d\n", 				   \
			desc, call, cudaGetErrorString(r), __FILE__, __LINE__);				   \
		return VIRG_FAIL;													   \
	}}
...
typedef struct {
    ...
    /// absolute ptr to beginning of metadata in NVM
    void        *base_ptr;
...
typedef struct {
	...
	/// pointer to the persistent memory for the open database
	char		*dbfd;
	/// name of persistent file
...
int virg_db_write_tab(virginian *v, virg_tablet_meta *tab, cudaStream_t *stream = NULL);
...
int virg_table_insert(virginian *v, unsigned table_id, char *key,
	char *data, char *blob);
int virg_table_insert_multiple(virginian *v, unsigned table_id, char *key, char *data, int data_size, 
	int rows, char *blob, double &operation_time, double &ddio_time, double &persist_time);
void virg_recover_insert(virginian *v, double *timer = NULL);
int virg_table_update_range(virginian *v, unsigned table_id, int row_start, 
    int row_end, char *data, int data_size, int column, double *timer = NULL);
int virg_table_update(virginian *v, unsigned table_id, int *selected_rows, int num_rows, char *data, 
	int data_size, int column, double &operation_time, double &ddio_time, double &persist_time, bool device_mem = false);
void virg_recover_update(virginian *v, double *timer = NULL);
...
int virg_vm_execute(virginian *v, virg_vm *vm, void* output = NULL, unsigned *get_rows = NULL);
...
int virg_vm_gpu(virginian *v, virg_vm *vm, virg_tablet_meta **tab_,
	virg_tablet_meta **res, unsigned num_tablets, void *output = NULL, unsigned *get_rows = NULL);
...
int virg_reader_copy(virginian *v, virg_reader *r, void *output);
...
int virg_query(virginian *v, virg_reader **reader, const char *query, void *output = NULL, unsigned *get_rows = NULL);
