...
int virg_db_load(virginian *v, unsigned tablet_id, virg_tablet_meta **tab)
{
	...
	cudaError_t r = cudaMemcpy(v->tablet_slots[slot], v->dbfd + x, sizeof(virg_tablet_meta), cudaMemcpyDeviceToHost);
	VIRG_CHECK(r != cudaSuccess, "Failed to get tablet meta data")
	v->tablet_slots[slot]->base_ptr = v->dbfd + x;
/*
	// get the rest of the tablet from disk
	r = cudaMemcpy((char*)v->tablet_slots[slot] + sizeof(virg_tablet_meta),
		v->dbfd + x + sizeof(virg_tablet_meta),
...
#endif
	VIRG_CHECK(r != cudaSuccess, "Failed to get tablet data")
*/
