...
{
	if(r->res == NULL || r->res->rows == 0)
		return VIRG_FAIL;

	virg_tablet_meta *res = r->res;
	char *dest = &r->buffer[0];
	unsigned i = 0;
	char *fixed = (char*)res->base_ptr + res->fixed_block;
	...
	for(i = 0; i < res->fixed_columns; i++) {
		...
		cudaMemcpy(dest, src, stride, cudaMemcpyDeviceToHost);
...
int virg_reader_copy(virginian *v, virg_reader *r, void *output)
{
	...
	while(1) {
		...
		for(i = 0; i < res->fixed_columns; i++) {
			size_t stride = res->fixed_stride[i];
			void *src = fixed + res->fixed_offset[i];
			cudaMemcpy((char*)output + offset, src, res->rows * stride, cudaMemcpyDeviceToHost);
			offset += res->rows * stride;
		}
